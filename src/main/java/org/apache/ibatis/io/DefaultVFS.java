...
public class DefaultVFS extends VFS {
  ...
  protected URL findJarForResource(URL url) throws MalformedURLException {
    ...
    // 1. 如果这个URL的文件部分不包括主机和协议， 则可能指向一个jar文件
    try {
      for (;;) {
    ...
    //2. 获取该 jar 文件对应的字符串表示，也就是该jar文件的绝对路径
   ...
   *    读取 url 对应的流， 判断流中的前四个字节是否和jar包对应的魔数相同，如果相同则表示为jar包
   */
